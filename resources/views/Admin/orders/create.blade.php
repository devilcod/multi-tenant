    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order_code') ? 'is-invalid' : '' }}" type="text" name="order_code" id="order_code" value="{{ old('order_code', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('table') ? 'is-invalid' : '' }}" name="table_id" id="table_id" required>
                    @foreach($tables as $id => $table)
                        <option value="{{ $id }}" {{ old('table_id') == $id ? 'selected' : '' }}>{{ $table }}</option>
                    @endforeach
                </select>
                @if($errors->has('table'))
                    <div class="invalid-feedback">
